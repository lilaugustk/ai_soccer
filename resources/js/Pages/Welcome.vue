<template>
  ...
  <MainLayout>
    <div class="py-8 relative isolate">
      <!-- AI Background Blobs -->
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="flex flex-col gap-6 mb-8">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-black flex items-center gap-3 text-gray-900 dark:text-white uppercase tracking-tight">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 relative">
                    <div class="flex items-center gap-2 bg-gray-100/50 dark:bg-gray-800/50 p-1.5 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-sm shadow-sm">
                        ...
                        <div class="relative">
                            ...
                            <div v-if="showDatePicker" class="absolute top-full left-0 mt-2 z-[60] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 p-4 w-72">
                                <div class="flex items-center justify-between mb-4">
                                    <span class="text-[10px] font-black uppercase text-gray-400 tracking-widest">{{ dayjs(props.filters.date).format("MMMM YYYY") }}</span>
                                ...
                                <div class="grid grid-cols-7 gap-1 mb-4">
                                    <span v-for="d in ['S','M','T','W','T','F','S']" :key="d" class="text-center text-[8px] font-black text-gray-300">{{ d }}</span>
                                    <button v-for="day in calendarDays" :key="day.date" @click="changeDate(day.date); showDatePicker = false;"
                                            ...
                                            :class="[day.isCurrentMonth ? '' : 'opacity-20', filters.date === day.date ? 'bg-emerald-500 text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300']">
                                        {{ day.dayNum }}
                                    </button>
                                </div>
                                <button @click="changeDate(today); showDatePicker = false;" class="w-full py-2 bg-gray-50 dark:bg-gray-700 rounded-xl text-[9px] font-black uppercase tracking-widest text-emerald-600">Hôm nay</button>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-1 overflow-x-auto no-scrollbar max-w-[200px] sm:max-w-[400px]">
                            <button v-for="btn in dateSlider" :key="btn.date" @click="changeDate(btn.date)"
                                    ...
                                    :class="filters.date === btn.date ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-400 hover:text-emerald-500'">
                    ...
                    <div v-if="availableLeagues.length > 0" class="flex gap-1.5 overflow-x-auto no-scrollbar flex-1">
                        <button @click="changeLeague(null)"
                                class="px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border whitespace-nowrap"
                        ...
                        <button v-for="league in availableLeagues" :key="league.id" @click="changeLeague(league.id)"
                                class="px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border flex items-center gap-1.5 whitespace-nowrap"
            ...
            <div v-if="Object.keys(groupedGames).length > 0" class="space-y-10">
                <div v-for="(games, leagueName) in groupedGames" :key="leagueName">
                    ...
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h2 class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] flex items-center gap-2">
                            {{ leagueName }}
                            <span class="text-[9px] font-black px-1.5 py-0.5 bg-gray-100 dark:bg-gray-900 rounded-md">{{ games.length }}</span>
                        </h2>
                    </div>
            ...
            <div v-else class="flex flex-col items-center justify-center py-24 text-center bg-white/30 dark:bg-gray-800/20 rounded-[3rem] border border-dashed border-gray-200 dark:border-gray-700">
                ...
                <h3 class="text-lg font-black mb-2 uppercase tracking-tight text-gray-900 dark:text-white">Không có dữ liệu</h3>
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest max-w-xs mx-auto">Chưa có trận đấu nào được nạp cho ngày {{ dayjs(filters.date).format('DD/MM') }}</p>
                <button @click="changeDate(today)" class="mt-8 px-6 py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[10px] font-black uppercase tracking-widest rounded-xl hover:translate-y-[-2px] transition-all">VỀ HÔM NAY</button>
            </div>
        </div>

        <!-- 3. Sidebar: Top Scorers & Hot Leagues -->
        <div class="space-y-12 shrink-0 w-full lg:w-80">
           <!-- Countries & Leagues Accordion -->
           <div>
              <h2 class="text-xl font-black mb-6 flex items-center gap-3 text-gray-900 dark:text-white uppercase tracking-tight">
                <div class="w-1.5 h-6 bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.3)]"></div>
                Quốc gia & Giải đấu
              </h2>
              
              <div class="space-y-1.5 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                 <div v-for="group in sortedLeagues" :key="group.country_name" class="group">
                    <!-- Country Header -->
                    <button @click="toggleCountry(group.country_name)"
                            :class="[
                                'w-full flex items-center justify-between p-3 rounded-xl transition-all border outline-none',
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            <!-- Country Flag Icon -->
                            <div class="w-7 h-7 rounded bg-white dark:bg-gray-900 flex items-center justify-center border border-gray-100 dark:border-gray-700 overflow-hidden shrink-0 shadow-sm relative">
                                <img :src="getFlagUrl(group.country_name)" 
                                     class="w-full h-full object-cover"
                                     @error="(e) => (e.target.style.display = 'none')" />
                                <span class="absolute inset-0 flex items-center justify-center text-[9px] font-black pointer-events-none" 
                            ...
                            <span class="text-xs font-bold truncate">{{ getLocalizedCountryName(group.country_name) }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span :class="['text-[9px] font-black px-1.5 py-0.5 rounded', expandedCountry === group.country_name ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-700 text-gray-500']">
                                {{ group.leagues.length }}
                            </span>
                            <svg :class="['w-3 h-3 transition-transform duration-300', expandedCountry === group.country_name ? 'rotate-180' : '']" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div v-show="expandedCountry === group.country_name" class="mt-2 ml-4 space-y-1 overflow-hidden transition-all">
                        <Link v-for="league in group.leagues" :key="league.id" :href="`/leagues/${league.id}`"
                              class="flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-colors group/item">
                            <div class="w-6 h-6 rounded-md bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 flex items-center justify-center p-1 shrink-0 group-hover/item:border-emerald-300 transition-colors">
                                <img v-if="league.logo_url" :src="league.logo_url" class="w-full h-full object-contain" />
                                <span v-else class="text-[8px] font-black text-emerald-600">{{ league.name.substring(0,2).toUpperCase() }}</span>
                            </div>
...
const getFlagUrl = (rawName) => {
    ...
    const flagCode = mapping[rawName] || rawName.toLowerCase().substring(0, 2);
